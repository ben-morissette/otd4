...
export default async function handler(req, res) {
  ...
  if (!team || !season) return res.status(400).json({ error: "team and season required" });

  const rarityMultiplier = RARITY_MULTIPLIERS[rarity] || 1;
  const regularUrl = `http://site.api.espn.com/apis/site/v2/sports/hockey/nhl/teams/${team}/schedule?season=${season}&seasontype=2`;
  ...
  const calcRax = (homeScore, awayScore, winner, type) => {
    ...
    if (winner === teamName) {
      return type === "Playoffs" ? 20 + diff * 20 : diff * 12;
    }
    return 0;
  };
